-- Date-wise attendance status codes for calendar-style marking.
-- Codes:
-- WO (Week Off), HO (Holiday), CO (Company Off), HD (Half Day),
-- CL (Casual Leave), SL (Sick Leave), PL (Paid Leave), LOP (Loss of Pay)
...
alter table public.attendance_logs
  ...
  check (
    ...
    or attendance_status in ('WO', 'HO', 'CO', 'HD', 'CL', 'SL', 'PL', 'LOP')
  );
...
begin
  ...
  if p_attendance_status is null
     or p_attendance_status not in ('WO', 'HO', 'CO', 'HD', 'CL', 'SL', 'PL', 'LOP') then
