-- ---------------------------------------------------------------------------
-- Cleanup: drop every overload of add_stock_to_store so PostgREST has a
-- single, unambiguous function to dispatch to. After all the iterations
-- (legacy 5-arg, vendor-added 6-arg with different param orders, etc.),
-- multiple versions can end up installed in the database at once, and
-- PostgREST refuses to pick one when called with named arguments — it
-- surfaces as a confusing "ambiguous" error (often naming the first
-- parameter it tried to match, e.g. "invoice_number").
--
-- Nothing in the DB depends on this function — it's only called from the
-- frontend "Add Stock" dialog — so dropping + recreating is safe.
-- ---------------------------------------------------------------------------

-- Drop every known signature.
...
-- Belt-and-braces: iterate pg_proc and drop any remaining overload, no
-- matter what the signature is. This catches any exotic param order left
-- behind from a partial migration run.
DO $$
DECLARE
  r record;
BEGIN
  FOR r IN
    SELECT oid::regprocedure::text AS sig
      FROM pg_proc
     WHERE proname = 'add_stock_to_store'
...
-- Recreate the canonical version (same body as 20260512000001).
...
BEGIN
  ...
  SELECT vendors.vendor_name, vendors.is_active
    INTO v_vendor_name, v_vendor_active
    FROM public.vendors
   WHERE vendors.vendor_id = p_vendor_id;
