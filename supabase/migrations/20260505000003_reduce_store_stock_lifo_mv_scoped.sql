-- ============================================================================
-- reduce_store_stock_lifo — MV-scope optional + richer logs.
--
-- Before (20260502000007): took (material, quantity, reason), LIFO-walked all
-- batches across every packaging + price variant of the material, and logged
-- a flat note without MV / units context.
--
-- After:
--   * Optional p_qty_variant_id — when set, LIFO is scoped to batches whose
--     price variant belongs to that packaging. NULL = walk every variant
--     (old behaviour), so the existing UI caller keeps working.
--   * Richer movement_log note: material name + metric, MV name when set,
--     per-batch lines with units + unit_price + variant name.
-- ============================================================================
...
BEGIN
  ...
  -- Availability check (scoped to MV if given).
  ...
  -- LIFO across batches (newest-arrival first), scoped to MV if given.
  FOR r IN
