-- ============================================================================
-- Replace project_costing_summary material_cost_actual source
-- from project_stock_used → allocation_variant_breakdown (FIFO).
--
-- Run AFTER 20260502000002_fifo_enhanced_logs_and_inventory_view.sql.
--
-- The old computation was:
--   sum((quantity_used / quantity_per_unit) * cost_per_unit) per project
-- using project_stock_used joined to material_variants (pack size).
--
-- The new computation is the exact FIFO cost:
--   sum(qty_used * unit_price) from allocation_variant_breakdown
-- joined to material_allocations for the project. No averaging, no pack
-- math — every row carries its exact FIFO unit price.
-- ============================================================================
...
-- NEW: FIFO-backed material cost. Exactly the sum of qty_used * unit_price
-- per breakdown row, rolled up by project.
fifo_material AS (
  SELECT
    ma.project_id,
...
-- Fallback for projects with no FIFO allocations but with legacy
-- material_movements ledger entries (Market Purchase etc.).
